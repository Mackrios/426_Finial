...
architecture testbench of tb_mips_pipeline is
  ...
  -- MANUALLY ENTER YOUR OPCODES HERE (from opcodes.txt)
  -- Format: Each opcode as hex value
  type opcode_array is array (0 to MAX_INSTRUCTIONS-1) of unsigned(15 downto 0);
  
  signal opcodes : opcode_array := (
    -- EDIT: Replace these with your opcodes from opcodes.txt
    -- Example: 0x0456, 0x1450, etc.
    0 => x"0060",  -- Opcode 0
    1 => x"1228",  -- Opcode 1
    2 => x"24F0",  -- Opcode 2
    3 => x"34F8",  -- Opcode 3
...
begin
  ...
  stim_process: process
    ...
    variable num_opcodes : integer := 8;  -- UPDATE THIS WITH YOUR OPCODE COUNT
  begin
    rst <= '1';
    wait for CLK_PERIOD * 2;
    rst <= '0';
